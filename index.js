...
(async function() {
    ...
    function normalizeThemeId(value) {
        ...
        return result.replace(/[?#].*$/, '').replace(/\.(?:css|less|json)$/i, '').trim();
    }
    ...
    function normalizeThemeBindings(bindings) {
        ...
        Object.entries(bindings).forEach(([rawId, rawBinding]) => {
            ...
            const themeId = normalizeThemeId(binding.themeId || rawId);
            if (!themeId) return;
            normalized[themeId] = {
                themeId,
    ...
    function normalizePresets(presets) {
        ...
        return presets.map((rawPreset, index) => {
            ...
            usedIds.add(id);
            const themeId = normalizeThemeId(preset.themeId || '');
            const isActive = !!preset.isActive && !!themeId && !activeThemes.has(themeId);
            if (isActive) activeThemes.add(themeId);
            return {
                id,
                ...
                themeId,
                themeName: String(preset.themeName || preset.themeId || '').trim(),
                isActive,
    ...
    function createPresetSnapshot(data, name, id = createPresetId()) {
        ...
        return {
            ...
            themeName: String(theme.name || theme.id || '').trim(),
            isActive: true,
            updatedAt: Date.now()
        };
    }
    ...
    function hasLegacyData(data) {
        ...
        return Object.keys(data?.themeBindings || {}).length > 0 ||
            (Array.isArray(data?.presets) && data.presets.some(preset => hasSettings(preset) || !normalizeThemeId(preset?.themeId)));
    }

    function mergeLegacyBindingsIntoPresets(presets, bindings, pseudoTarget) {
        const result = normalizePresets(presets);
        Object.values(normalizeThemeBindings(bindings)).forEach(binding => {
            let preset = result.find(item => item.themeId === binding.themeId);
            if (!preset) {
                preset = result.find(item => !item.themeId &&
                    (item.activeUserSrc || null) === (binding.userFrameSrc || null) &&
                    (item.activeCharSrc || null) === (binding.charFrameSrc || null));
            }
            if (!preset) {
                preset = {
                    id: createPresetId(),
                    name: `${binding.themeName}（原绑定）`,
                    activeUserSrc: binding.userFrameSrc || null,
                    activeCharSrc: binding.charFrameSrc || null,
                    pseudoTarget: pseudoTarget === 'before' ? 'before' : 'after',
                    updatedAt: Number(binding.updatedAt) || Date.now()
                };
                result.push(preset);
            }
            preset.themeId = binding.themeId;
            preset.themeName = binding.themeName;
    ...
    function normalizeFrameData(data) {
        ...
        data.presets = mergeLegacyBindingsIntoPresets(data.presets, data.themeBindings, data.pseudoTarget);
        delete data.themeBindings;
        const currentTheme = getCurrentThemeSnapshot();
        data.presets.forEach(preset => {
            if (!preset.themeId) {
                preset.themeId = normalizeThemeId(currentTheme.id);
        ...
        if (activePreset && presetMatchesData(activePreset, data)) {
            ...
            if (activePreset) activePreset.isActive = false;
    ...
    function getCurrentThemeSnapshot() {
        ...
        const $themeSelect = $('#themes, #theme').filter(function() { return String($(this).val() || '').trim() !== ''; }).first();
        const selectedId = $themeSelect.length ? normalizeThemeId($themeSelect.val()) : '';
        const selectedName = $themeSelect.length ? String($themeSelect.find('option:selected').text() || '').trim() : '';
        const themeHref = String($('link[href*="/themes/"]').first().attr('href') || '');
        const themeMatch = themeHref.match(/\/themes\/([^/?#]+)/i);
        const linkedThemeId = themeMatch ? normalizeThemeId(themeMatch[1]) : '';
        const settingsThemeId = normalizeThemeId(
            (typeof settings !== 'undefined' && settings.visual_theme) ||
            context.powerUserSettings?.visual_theme || context.powerUserSettings?.theme ||
            context.power_user?.visual_theme || context.power_user?.theme ||
            window.power_user?.visual_theme || window.power_user?.theme || ''
        );
        const detected = !!(selectedId || linkedThemeId || settingsThemeId);
        const themeId = selectedId || linkedThemeId || settingsThemeId || 'Default_Theme';
        const themeName = String(selectedName || themeId).trim() || themeId;
        return { id: themeId, name: themeName, detected };
    }
    ...
    const LocalDataStore = {
        ...
        load: async function() {
            ...
            const needsMigration = hasLegacyData(data);
    ...
    const DataManager = {
        ...
        _loadServer: async function() {
            ...
            const needsMigration = hasLegacyData(result.data);
            ...
            if (!data.schemaVersion || pendingSync) {
                ...
                const hasLocalData = localData.userFrames.length > 0 || localData.charFrames.length > 0 || localData.presets.length > 0;
...
.mes:not([is_user]) .avatar::${pseudo} { ${charRule} }`;
    ...
    async function syncThemeBinding() {
        const theme = getCurrentThemeSnapshot();
        if (!theme.id || theme.id === lastThemeBindingId) return;
        ...
        }).catch(error => console.warn('[头像框管理器] 美化预设同步失败', error));
    ...
    function installThemeBindingWatcher() {
        ...
        const scheduleSync = () => {
            const expectedTheme = getCurrentThemeSnapshot();
            const expectedThemeId = expectedTheme.id;
            if (!expectedThemeId || !expectedTheme.detected) return;
            $('.nsk-current-theme strong').text(expectedTheme.name || expectedThemeId);
        ...
        const bindThemeSelector = () => {
            $('#themes, #theme').each(function() {
                const $selector = $(this);
                if ($selector.data('afm-theme-watcher-bound')) return;
                $selector.data('afm-theme-watcher-bound', true).off('change.afmThemeBinding').on('change.afmThemeBinding', scheduleSync);
            });
            scheduleSync();
        };
        bindThemeSelector();
        window.__afmThemeBindingWatcherTimer = setInterval(bindThemeSelector, 1000);
    }
    ...
    $('head').append(`
        <style id="${STYLE_ID}">
            ...
            .nsk-title-block { min-width: 0; display: flex; flex-direction: column; gap: 4px; }
            .nsk-title { font-weight: bold; font-size: 1.1em; display: flex; align-items: center; gap: 8px; }
            .nsk-current-theme { font-size: 0.76em; opacity: 0.62; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
            .nsk-current-theme i { margin-right: 4px; }
            ...
            .afm-global-label { display: inline-flex; margin-left: 5px; padding: 2px 5px; border: 1px solid currentColor; border-radius: 5px; font-size: 0.68em; line-height: 1; opacity: 0.62; vertical-align: middle; }
        ...
        return `
            <div class="afm-settings-container">
                ...
                <div class="afm-setting-group">
                    <div class="afm-setting-header">
                        <span><i class="fa-solid fa-user"></i> User 头像框调整 (%) <small class="afm-global-label">全局固定</small></span>
                ...
                <div class="afm-setting-group">
                    <div class="afm-setting-header">
                        <span><i class="fa-solid fa-robot"></i> Char 头像框调整 (%) <small class="afm-global-label">全局固定</small></span>
                        <div class="afm-setting-actions">
    ...
    function createPresetsHTML(data) {
        const currentTheme = getCurrentThemeSnapshot();
        const presets = normalizePresets(data.presets).filter(preset => preset.themeId === currentTheme.id);
        ...
        const items = presets.length ? presets.map(preset => {
            const isActive = preset.isActive && preset.id === data.activePresetId && presetMatchesData(preset, data);
            return `
        ...
        return `
            <div class="afm-settings-container">
                <div class="afm-setting-group">
                    ...
                    <div class="afm-binding-current"><strong>当前美化：${escapeHTML(currentTheme.name || currentTheme.id)}</strong><code>预设按当前美化分别保存；记录 User、Char 头像框和伪元素设置，位置数值保持全局固定。</code></div>
    ...
    async function showManagerUI() {
            ...
            <div class="nsk-overlay ${isDarkMode ? 'afm-dark-mode' : ''}">
                <div class="nsk-box">
                    ...
                    <div class="nsk-header">
                        <div class="nsk-title-block">
                            <div class="nsk-title"><i class="fa-solid fa-crop-simple"></i> ${SCRIPT_NAME}<span class="nsk-version">v${SCRIPT_VERSION}</span></div>
                            <div class="nsk-current-theme"><i class="fa-solid fa-circle-info"></i> 当前主题：<strong>${escapeHTML(currentTheme.name || currentTheme.id)}</strong></div>
                        </div>
        ...
        function bindPresetEvents() {
            ...
            $popup.find('.afm-preset-switch').on('click', async function() {
                ...
                if (!preset) return;
                const isActive = preset.isActive && preset.id === currentData.activePresetId && presetMatchesData(preset, currentData);
                if (isActive) {
            ...
            $popup.find('.afm-preset-save').on('click', async function() {
                ...
                const ok = await showAFMConfirmDialog({ title: '覆盖头像框预设', message: `确定用当前 User、Char 头像框选择覆盖“${preset.name}”吗？全局位置数值不会写入预设。`, okText: '保存覆盖' });
                if (!ok) return;
                const replacement = createPresetSnapshot(currentData, preset.name, preset.id);
                currentData.presets.forEach(item => {
                    if (item.themeId === replacement.themeId) item.isActive = false;
                });
        ...
        await refreshGrids();
        bindPresetEvents();
        themeBindingAppliedHandler = async event => {
            currentData = await DataManager.load();
            const appliedTheme = event && event.detail ? event.detail : getCurrentThemeSnapshot();
            $popup.find('.nsk-current-theme strong').text(appliedTheme.name || appliedTheme.id || 'Default_Theme');
            renderRoleGrid(getActiveRole());
            renderSettings();
            renderPresets();
        };
            ...
            if (ok) {
                ...
                indicesArray.forEach(idx => {
                    if (list[idx]) {
                        ...
                        clearFrameFromPresets(currentData, isUser ? 'user' : 'char', deletedSrc);
                        list.splice(idx, 1);
                    }
                });
                await DataManager.save(currentData);
        ...
        $popup.find('#btn-clear-all').on('click', async function() {
            ...
            if (ok) {
                currentData = await DataManager.load();
                if (isUser) {
                    currentData.userFrames.forEach(frame => clearFrameFromPresets(currentData, 'user', frame.src));
                    currentData.userFrames = [];
                    currentData.activeUserSrc = null;
                } else {
                    currentData.charFrames.forEach(frame => clearFrameFromPresets(currentData, 'char', frame.src));
    ...
    window.__afmHotCleanup = () => {
        ...
        if (window.__afmThemeBindingWatcherTimer) clearInterval(window.__afmThemeBindingWatcherTimer);
        if (window.__afmThemeBindingDebounceTimer) clearTimeout(window.__afmThemeBindingDebounceTimer);
        $('#themes, #theme').removeData('afm-theme-watcher-bound').off('change.afmThemeBinding');
        $('.nsk-overlay').remove();
        $(`#${MENU_BTN_ID}`).remove();
        $(`#${STYLE_ID}`).remove();
        $(`#${APPLIED_STYLE_ID}`).remove();
